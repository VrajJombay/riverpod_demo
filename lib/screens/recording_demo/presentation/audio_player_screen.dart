      ...
      body: Center(
        child: Consumer(
          builder: (context, ref, child) {
            final canRecordOrPlay = ref.watch(audioPlayerProvider.select((value) => value.canRecordOrPlay));
            return Padding(
              ...
              child: canRecordOrPlay
                  ? isNetworkUrl
                      ? PlayerWidget(
                          ...
                          onDeleteIconClicked: () {
                            ref.read(audioPlayerProvider.notifier).canRecordOrPlay(false);
                          },
                        )
                      : Consumer(
                          builder: (context, ref, child) {
                            String? localFileUrl = ref.watch(audioPlayerProvider.select((value) => value.filePath));
                            print('localFileUrl--->$localFileUrl');
                            return ((localFileUrl?.isNotEmpty ?? false) && localFileUrl != null)
                                ? PlayerWidget(
                                    isNetworkUrl: isNetworkUrl,
                                ...
                                : RecorderWidget(
                                    recordingCompleted: (fileUrl) {
                                      WidgetsBinding.instance.addPostFrameCallback((callback) {
                                        ref.read(audioPlayerProvider.notifier).recordingCompleted(fileUrl);
                                      });
                                    },
                                    onCrossIconClicked: () {
                                      ref.read(audioPlayerProvider.notifier).canRecordOrPlay(false);
                  ...
                  : InkWell(
                      onTap: () {
                        ref.read(audioPlayerProvider.notifier).canRecordOrPlay(true);
                      },
